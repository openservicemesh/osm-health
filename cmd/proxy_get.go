...
func (cmd *proxyGetCmd) run() error {
	...
	if err != nil {
		return fmt.Errorf("Could not find pod %s in namespace %s", cmd.pod, cmd.namespace)
	}
	if !isMeshedPod(*pod) {
		return fmt.Errorf("Pod %s in namespace %s is not a part of a mesh", cmd.pod, cmd.namespace)
	}
	if pod.Status.Phase != corev1.PodRunning {
		return fmt.Errorf("Pod %s in namespace %s is not running", cmd.pod, cmd.namespace)
	}
	...
	if err != nil {
		return fmt.Errorf("Error retrieving proxy config for pod %s in namespace %s: %s", cmd.pod, cmd.namespace, err)
	}
